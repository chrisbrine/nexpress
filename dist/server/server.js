...
class NExpress {
  ...
  getHostString(port) {
    ...
    if (host) {
      const protocol =
        port.protocol === types_1.NExpressPortTypes.HTTPS ? "https" : "http";
  ...
  startServer(port) {
    const useHttps =
      port.protocol === types_1.NExpressPortTypes.HTTPS ? true : false;
    ...
    if (useHttps) {
      if (httpsOptions.key === "" || httpsOptions.cert === "") {
        ...
        https_1.default
          .createServer(certFiles, this.app)
          .listen(port.port, () => {
            utils_1.log.info(`Server started on ${this.getHostString(port)}`);
          });
      }
    } else {
